...
async function main() {
    ...
    const outputFileSegments = inputFile.split("/").map((s, i, a) => i === a.length - 1 ? s.replace(/\.tsx?$/, "") : s);
    let outputFile;
    ...
    try {
        ...
        run: {
            ...
            try {
                ...
                const parsed = ts.parseJsonConfigFileContent(configFile.config, ts.sys, path.dirname(synthTsConfigPath));
            ...
            const result = spawnSync("node", ["--import", "tsx", "--enable-source-maps", outputFile, ...nodeArgs], { stdio: "inherit" });
            if (result.signal) {
